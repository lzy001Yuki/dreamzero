    ...
    def _save_video_predictions(self) -> None:
        ...
        try:
            ...
            output_prefix = os.path.join(
                self._output_dir,
                f"{len(all_mp4_files):06}_{timestamp}_{safe_session}_frames{len(frame_list)}",
            )
            grid_path = f"{output_prefix}_grid.mp4"
            imageio.mimsave(grid_path, frame_list, fps=5, codec="libx264")
            logger.info("Saved infer video grid to: %s", grid_path)

            height, width = sample_frame.shape[:2]
            if height % 2 == 0 and width % 2 == 0:
                half_h = height // 2
                half_w = width // 2
                # For real_panda_single_arm/find9shape, _prepare_video lays out:
                #   top/head view   -> top-left quadrant
                #   wrist view      -> bottom-left quadrant
                top_frames = [frame[:half_h, :half_w] for frame in frame_list]
                wrist_frames = [frame[half_h:, :half_w] for frame in frame_list]
                top_path = f"{output_prefix}_top.mp4"
                wrist_path = f"{output_prefix}_wrist.mp4"
                imageio.mimsave(top_path, top_frames, fps=5, codec="libx264")
                imageio.mimsave(wrist_path, wrist_frames, fps=5, codec="libx264")
                logger.info("Saved infer top/head video to: %s", top_path)
                logger.info("Saved infer wrist video to: %s", wrist_path)
            else:
                logger.warning(
                    "Cannot split infer video into top/wrist views because decoded shape is %s",
                    sample_frame.shape,
                )
